...
{
    public partial class AddEditSaleItemForm : Form
    {
        ...
        public AddEditSaleItemForm(SaleItem? saleItem = null)
        {
            InitializeComponent();
            _productRepository = new ProductRepository();
            _saleItem = saleItem;
            _isEdit = saleItem != null;

            if (_isEdit)
            {
                this.Text = "ویرایش آیتم فروش";
                LoadSaleItemData();
            }
            else
            {
                this.Text = "افزودن آیتم فروش";
            }
        ...
        private void LoadSaleItemData()
        {
            if (_saleItem != null)
            {
                _cmbProduct!.Text = _saleItem.Product.Name;
                _numQuantity!.Value = _saleItem.Quantity;
                _numUnitPrice!.Value = _saleItem.UnitPrice;
                _numDiscountAmount!.Value = _saleItem.DiscountAmount;
                CalculatePrices();
            }
        }
            ...
            try
            {
                if (_cmbProduct!.SelectedIndex >= 0)
                {
                    ...
                    if (product != null)
                    {
                        ...
                        _numUnitPrice!.Value = product.Price;
                        CalculatePrices();
                    }
                }
            }
        ...
        private bool ValidateInput()
        {
            ...
            if (product != null && _numQuantity.Value > product.StockQuantity)
            {
                MessageBox.Show($"موجودی کافی نیست. موجودی فعلی: {product.StockQuantity}", "خطا", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                _numQuantity.Focus();
                return false;
            }
        ...
        {
            var saleItem = _saleItem ?? new SaleItem();
            ...
            if (product != null)
            {
                ...
                saleItem.Quantity = (int)_numQuantity!.Value;
                saleItem.UnitPrice = _numUnitPrice!.Value;
                saleItem.TotalPrice = saleItem.Quantity * saleItem.UnitPrice;
                saleItem.DiscountAmount = _numDiscountAmount!.Value;
                saleItem.FinalPrice = saleItem.TotalPrice - saleItem.DiscountAmount;
            }
